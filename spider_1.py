...
BASE_URL = "http://wx.zikaojihui.com/"
...
def start(userName, passWord, courseCode, url=BASE_URL, zhang=1, jie=2, hintCallback=None):
    ...
    while (has_next_section):
        ...
        while (has_next_jie):
            ...
            try:
                jie = parent.find_element_by_xpath(
                    parent_base_xpath + "/li[" + str(current_section_index) + "]/ul/li[" + str(
                        current_jie_index) + "]/p/a")
            ...
            while (has_next_video):
                try:
                    hint_ke = "获取第" + str(current_section_index) + "章，第" + str(current_jie_index - 1) + "节，第" + str(
                        current_video_index - 1) + "课"
                    print(hint_ke)
                    if hintCallback:
                        hintCallback(hint_ke)
                    video = video_parent.find_element_by_xpath(
                        parent_base_xpath + "/li[" + str(current_section_index) + "]/ul/li[" + str(
                            current_jie_index) + "]/ul/li[" + str(current_video_index) + "]/p/a[1]")
                    video_name = "{0}.{1}.{2}-{3}".format(current_section_index, current_jie_index - 1,
                                                          current_video_index - 1, video.get_attribute("title"))
                    ...
                    fetch(proxy, video,
                          video_name)  # //*[@id="app"]/div/div[2]/div[2]/div[1]/div[2]/div[2]/div[1]/div/div/div[1]/div/ul/li[1]/ul/li[2]/ul/li[2]/p/a[1]
        ...
        for index, line in enumerate(file_line):
            ...
            if "EXTINF" in line:
                ...
                with open(download_path + "\\" + c_fule_name, 'ab') as f:
                    cryptor = AES.new("9c861f057dcbe368".encode("utf-8"), AES.MODE_CBC,
                                      "a2a173a99896112c".encode("utf-8"))
...
start("13326974965", "Zk123456", "20253", url="http://wx.zikaojihui.com/", zhang=9)
